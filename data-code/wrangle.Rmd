...
my_update <- function(data_left, data_right) {
  for (i_right in 1:nrow(data_right)) {  # for each ID in right
    ...
    while (is.na(data_left[i_left,reason_s])[[1]] == F) { 
      suffix <- suffix+1
      reason_s <- paste0('reason',suffix)
    }
    vars = c('reason')
...
# if false
i_left <- which(df_left_test$ID == ID)
reason_s <- paste0('reason_short', suffix)
...
```{r demerger, 'demerged', 'duplicate', 'merger result', 'ID change'}
df_change %>% 
