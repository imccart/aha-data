...
```{r Merger}
aha_data %>%
  filter(readum_Merger==1 | readum_Result==1) %>%
  select(ID,year,add,del,reason)
```
...
```{r open}
aha_data %>%
  filter(readum_Newly==1) %>%
  select(ID,year,add,del,reason) %>%
  select(reason) %>%
  unique()
# added -> add
# registered -> delete data
```

```{r reopen}
aha_data %>%
  filter(readum_Reopened==1) %>%
  select(ID,year,add,del,reason)
```

```{r temporarily}
aha_data %>%
  filter(readum_Temporarily==1) %>%
  select(ID,year,add,del,reason)
```

```{r formerly}
aha_data %>%
  filter(readum_Formerly==1) %>%
  select(ID,year,add,del,reason)
```

```{r bug - hardfix}
aha_data %>%
  filter(readum_nNoenwrleyg==1) %>%
  select(ID,year,add,del,reason)
```

```{r i dun understand the reason}
aha_data %>%
...
```{r duplicate}
aha_data %>%
  filter(readum_Duplicate==1) %>%
  select(ID,year,add,del,reason)
```
```{r del-Unit of other hospital}
aha_data %>%
  filter(readum_Unit==1) %>%
  select(ID,year,add,del,reason)
```
```{r ?? bug}
aha_data %>%
